package de.hdodenhof.circleimageview.sample.utils;
...
public class AnimationDrawableUtils {
    ...
    //
    private static void loadRaw(final int resourceId, final ImageView imageView,
                                final OnDrawableLoadedListener onDrawableLoadedListener) {
        loadFromXml(resourceId, imageView, onDrawableLoadedListener);
    }
    ...
    private static void loadFromXml(final int resourceId,
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Resources resources = (Resources) imageView.getTag();
                XmlResourceParser parser;
                if (resources != null) {
                    parser = resources.getXml(
                            resourceId);
                } else {
                    parser = imageView.getContext().getResources().getXml(
                            resourceId);
                }
                                ...
                                for (int i = 0; i < parser.getAttributeCount(); i++) {
                                    if (parser.getAttributeName(i).equals(
                                        ...
                                        int resId = Integer.parseInt(parser
                                                ...
                                                .substring(1));
                                        if (resources != null) {
                                            bytes = IOUtils.toByteArray(resources
                                                    .openRawResource(resId));
                                        } else {
                                            bytes = IOUtils.toByteArray(imageView.getContext()
                                                    .getResources()
                                                    .openRawResource(resId));
                                        }
